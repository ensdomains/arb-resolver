...
const StubAbi = require('../../contracts/artifacts/contracts/l1/ArbitrumResolverStub.sol/ArbitrumResolverStub.json').abi
const IResolverAbi = require('../../contracts/artifacts/contracts/l1/ArbitrumResolverStub.sol/IResolverService.json').abi
const program = new Command();
const { defaultAbiCoder, hexConcat } = require("ethers/lib/utils");
program
  .requiredOption('-r --registry <address>', 'ENS registry address')
  .option('-l1 --l1_provider_url <url1>', 'L1_PROVIDER_URL', 'http://localhost:8545')
  .option('-l2 --l2_provider_url <url2>', 'L2_PROVIDER_URL', 'http://localhost:8547')
